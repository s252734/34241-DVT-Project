...
# Keep the same working thresholds from the v2 logic
...
# Non PV object rejection threshold
# This is only used as a rejection flag.
# It does not modify panel coverage.
NON_PV_COMPONENT_AREA_THRESHOLD = 0.035
NON_PV_EDGE_DENSITY_THRESHOLD = 0.055
...
def estimate_panel_coverage_thermal(panel_crop):
    ...
    Important:
    This mask is used for panel coverage and thermal statistics.
...
def detect_non_pv_hot_object(panel_crop, hotspot_mask):
    ...
    Important:
    This function does not change the panel mask.
    It only returns a rejection flag.

    It is intended to catch the black plate / reference object
    that appears hot but is not a solar panel.
    """
    ...
    for label in range(1, num_labels):
        ...
        large_hot_region = component_ratio > NON_PV_COMPONENT_AREA_THRESHOLD
        weak_texture = edge_density < NON_PV_EDGE_DENSITY_THRESHOLD

        if large_hot_region and weak_texture:
...
def clean_hotspot_mask_for_overlay(hotspot_mask, panel_crop):
    ...
    This does not affect panel coverage or hotspot statistics.
    """
...
analysis_csv_path = RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_analysis.csv"
summary_csv_path = RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_summary.csv"
...
best_sharpness.to_csv(RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_best_sharpness_frames.csv", index=False)
worst_sharpness.to_csv(RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_worst_sharpness_frames.csv", index=False)
highest_coverage.to_csv(RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_highest_coverage_frames.csv", index=False)
lowest_coverage.to_csv(RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_lowest_coverage_frames.csv", index=False)
highest_contrast.to_csv(RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_highest_hotspot_contrast_frames.csv", index=False)
lowest_contrast.to_csv(RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_lowest_hotspot_contrast_frames.csv", index=False)
largest_hot_area.to_csv(RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_largest_hot_area_frames.csv", index=False)
rejected_non_pv_hot.to_csv(RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_rejected_non_pv_hot_frames.csv", index=False)
best_inspection_frames.to_csv(RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_best_inspection_frames.csv", index=False)
still_vs_video_examples.to_csv(RESULT_DIR / f"{VIDEO_NAME}_thermal_fixed_still_vs_video_examples.csv", index=False)
...
plt.grid(True)
plt.tight_layout()
plt.savefig(PLOT_DIR / f"{VIDEO_NAME}_thermal_fixed_sharpness_over_time.png", dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(PLOT_DIR / f"{VIDEO_NAME}_thermal_fixed_panel_coverage_over_time.png", dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(PLOT_DIR / f"{VIDEO_NAME}_thermal_fixed_junction_over_time.png", dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(PLOT_DIR / f"{VIDEO_NAME}_thermal_fixed_statistics_over_time.png", dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(PLOT_DIR / f"{VIDEO_NAME}_thermal_fixed_hotspot_contrast_over_time.png", dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(PLOT_DIR / f"{VIDEO_NAME}_thermal_fixed_hot_area_percentage_over_time.png", dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(PLOT_DIR / f"{VIDEO_NAME}_thermal_fixed_non_pv_hot_rejection.png", dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(PLOT_DIR / f"{VIDEO_NAME}_thermal_fixed_usable_frames_over_time.png", dpi=300)
plt.close()
